...
#include <time.h>
    ...
    //Função para trabalho de xp, executada na MAIN.
    int xp = 0;
    ...
    int main () {
        ...
        if (vidainimigo1 < 0) {
            ...
            moedas = fortunewheel;
            printf("\nVocê logrou exito no combate contra o morcego, por isso recebera moedas: %d\n", rand() % 10 + 1);
            xp += rand() % 5 + 15; // Ganho de experiência
        ...
        if (vidainimigo2 < 0)
        {
            fortunewheel = rand() %10 + 1;
            moedas = fortunewheel;
            printf("\nVocê logrou exito no combate contra o rato, por isso recebera moedas: %d\n", rand() % 10 + 1);
            xp += rand() % 5 + 15; // Ganho de experiência
            printf("\nTambem sera acrescentado %d de exp devido a vitoria\n", xp);
        }
        else if (inimigo == 3)
        ...
        if (vidainimigo3 < 0) {
            fortunewheel = rand() % 10 + 1;
            moedas = fortunewheel;
            printf("\nVocê logrou exito no combate contra o urso, por isso recebera moedas: %d\n", rand() % 10 + 1);
            xp += rand() % 5 + 15; // Ganho de experiência
            printf("\nTambem sera acrescentado %d de exp devido a vitoria\n", xp);
        }
        else
        ...
        if (vidainimigo4 < 0)
        {
            fortunewheel = rand() % 10 + 1;
            moedas = fortunewheel;
            printf("\nVocê logrou exito no combate contra o dragao, por isso recebera moedas: %d\n", rand() % 10 + 1);
            xp += rand() % 5 + 15; // Ganho de experiência
            printf("\nTambem sera acrescentado %d de exp devido a vitoria\n", xp);
        }
        return 0;
